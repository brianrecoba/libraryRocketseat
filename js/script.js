...
function handleSubmit(event) {
    ...
    const currentBooks = rawData ? JSON.parse(rawData) : [];
    ...
    const bookData = {
        ...
        urlImage : document.getElementById('url').value.trim(),
        status: document.getElementById('status').value,
        avaliation: ratingValue,
...
function createModalStars() {
    ...
    for (let i = 5; i >= 1; i--) {
        starsHtml +=
        `
            <input type="radio" id="modal-star-${i}" name="avaliation" value="${i}">
            <label for="modal-star-${i}">
            <span class="star-empty">${startEmpty}</span>
            <span class="star-filled">${starFilled}</span>
            </label>
        `
            ;
    }
...
function getAllBooks() {
    ...
    bookContainer.classList.remove('no-book-container')
    bookContainer.classList.remove('container')
...
function deleteBook(id){
    ...
    const newBook = storageBook.filter(item => item.id !== idToNumber)
...
function renderBooks(booksList) {
     ...
     booksList.forEach(book => {
        bookContainer.innerHTML += `
        <div class="book-card">
                <img src="${book.urlImage || defaultImage }" class="book-image" alt="Capa do livro ${book.title}" onerror="this.src='${defaultImage}'; this.onerror=null; "/>
                <h3>${book.title}</h3>
                <p class="book-author">${book.author}</p> 
                <div class="book-status" data-status="${book.status}">
                    <span>${book.status}</span>
                </div>
                <div class="book-rating">
                    ${renderStart(Number(book.avaliation))}
                </div>
